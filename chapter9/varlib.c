 ...
 */
{
	...
	retval = malloc( strlen(name) + strlen(val) + 2 ); // 一个等号，一个'\0'
	if ( retval != NULL )
 ...
 */
{
	...
	if ( (itemp = find_item(name,0)) != NULL )
		return itemp->str + 1 + strlen(name); // 通过指针偏移返回value
	return "";

}

// 将shell变量变成用户变量（通过设置boolean）
...
// tab中存储的是name=val的字符串;如果找不到且（first_blank），就返回第一个空的位置
